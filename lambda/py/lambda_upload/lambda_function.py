...
class LaunchRequestHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        if not playback_info.get('has_previous_playback_session'):
            ...
            current_track = util.get_current_track(handler_input)
...
#TODO: Gracefully handle shuffle on handler
...
class PlaybackStartedEventHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        playback_info["current_id"] = util.get_current_track(handler_input).guid
...
class PlaybackNearlyFinishedEventHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        next_track = util.get_next_track(handler_input)    
        
        playback_info["next_stream_enqueued"] = True
        enqueue_token = next_track.guid
        ...
        handler_input.response_builder.add_directive(
            PlayDirective(
                ...
                audio_item=AudioItem(
                    stream=Stream(
                        token=enqueue_token,
                        url=next_track.enclosures[0].url,
...
class LoadPersistenceAttributesRequestInterceptor(AbstractRequestInterceptor):
    ...
    def process(self, handler_input):
        ...
        if len(persistence_attr) == 0:
            ...
            playback_info["current_id"] = playback_info.get("current_id")
